...
#  Runs the main control loop
    ...
    while elapsed_time <=3:
        ...
        await asyncio.sleep(1)
        elapsed_time = time.time() - start_time
        # An if statment that causes the right headlight to flash and drive forward before turning
        if elapsed_time >=1:
             ...
             await asyncio.sleep(1)
             break
    # after around 2-4 seconds drive, turn right.
    ...
    await asyncio.sleep(1)
    print("turning right")
    await rvr.reset_yaw()  # adjust current heading
   # Delay
    await asyncio.sleep(1)
  ...
  #dirve straight through our doorway
    await asyncio.sleep(1)
    await rvr.drive_with_heading(speed=90,heading=0,flags=0)
